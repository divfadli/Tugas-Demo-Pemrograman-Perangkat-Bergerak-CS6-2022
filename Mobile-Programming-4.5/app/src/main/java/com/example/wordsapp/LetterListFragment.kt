...
class LetterListFragment : Fragment() {
    ...
    private fun chooseLayout() {
        recyclerView.layoutManager = if (isLinearLayoutManager) LinearLayoutManager(context) else GridLayoutManager(context, 4)
        recyclerView.adapter = LetterAdapter()
    }

    private fun setIcon(menuItem: MenuItem?) {
        if (menuItem == null) return
        menuItem.icon = ContextCompat.getDrawable(requireContext(), if (isLinearLayoutManager) R.drawable.ic_grid_layout else R.drawable.ic_linear_layout)
    }
